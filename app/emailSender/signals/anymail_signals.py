...
def handle_status_change(sender, event, esp_name, **kwargs):
    if event.event_type in ['autoresponded', 'opened', 'clicked', 'complained', 'unsubscribed', 'subscribed']:
            return
    try:
        print('Received webhook signal for an email status update.')
        sent_email = SentEmail.objects.get(message_id=event.message_id, recipient=event.recipient)
        sent_email.status = get_status_choices_name(event.event_type)
        sent_email.save()
        print(f'Updated status of message {event.message_id} and recipient {event.recipient} to {sent_email.status}')
